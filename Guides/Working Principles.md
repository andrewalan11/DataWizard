...
edit_log:
  ...
  - 'DW-S330 2026-09-04 - Additional principles: check whether the rule already exists before minting it (named on second use, per the S324 codify-on-second-use marker; instances S324 + S330)'
  - "DW-S374 2026-09-23 - named the verify-before-redo / re-read-the-charter
    concurrency principle (meta-learning review S301-S323)"
  - "DW-S374 2026-09-23 - generalized the check-the-rule-exists block to
    inspect-the-ground (meta-learning review S324-S337)"
  - "DW-S374 2026-09-23 - named the completion-is-a-disk-fact and
    durable-home-on-receiver's-path principles; countable-gate sentence on
    Ground-truth (meta-learning reviews S301-S362)"
---
# Working Principles
...
**Ground-truth the substrate.** Don't trust the artifact - read what it stands on. One stance, three canonical expressions, each maintained in its own home: for *data*, the pre-flight distribution check (the review-before-live standing questions in a project's 0.0 - does the flag correlate with a metadata attribute rather than content?); for *builds*, the verification half of the supervised-build skill (verify on disk, run shipped scripts on fixtures, never trust the report); for *concurrent writes*, the Conventions Registry's optimistic-claim pattern (verify the token, and not just once). Named once here so recurring instances count against a standing principle instead of re-deriving the family; the rules themselves live and evolve in those three homes. (DataWizard, 2026-08) A fourth expression, for *self-assessment*: any step where a model reviews its own output is a candidate for a countable gate - scan and write counts ("routed=7", "3 of 3 checks named"), never "looks clean". (DataWizard, 2026-09)

**On high-concurrency days, verify sibling state before redoing - and re-read the charter, not just the target.** When several instances work the same project in parallel, the cheapest defect to ship is a duplicate: a "fix" of something a sibling already fixed, a rebuild of a surface a sibling already restructured, a stub or tracker written over in-progress work. Before redoing or repairing anything on a day with active siblings, re-read the charter or driver doc (not only the artifact you are about to touch) and check the target's current on-disk state for a sibling's hand. Re-logged five times in one scan window after becoming practice; named here so instances count against a standing principle instead of re-deriving it per session. (DataWizard, 2026-08 to 2026-09)

**A completion is a disk fact, not a claim.** Every "done", "deployed", "swept", "handed off" - and every *declaration* a behavior keys off (a registry, a catalog row, a board summary) - needs the probe that would falsify it: a zero-residual check, an artifact-exists check, or a lint of the declaration against the on-disk reality it claims to describe. The failure modes are one family, observed a dozen ways across two scan windows: a fix "in flight" existing nowhere on disk; a logged sweep with sixteen residuals surviving; a hand-off with no receiving artifact; an unfalsifiable "deployed" claim on a config no session can read; a mode switch keyed off a declared operator registry that ran a project wrong for two months with zero errors; a summary line reading "clear" while the row it summarizes read "to do". Named once here; the working rules live in their own homes - the consumer's-path principle above (the delivery half), Ground-truth the substrate (the verification stance), the pre-flight standing questions (a project's 0.0), and the lint enforcement arm (a project's staleness-detection checks). (DataWizard, 2026-08 to 2026-09)

**Capture and handoff need a durable home on the receiver's path.** Two halves of one leak. Capture: a queue, net, or write-obligation without a home that *owns* it drips work into exchange notes and session logs where no cold reader looks - pair every net with a home that owns the write. Handoff: a surface the receiver never opens is not a handoff, however correct its content - the artifact lives where the *receiver* orients, and a producer should know whether its consumer exists (write-succeeds-with-no-reader is a silent-failure class at every scope level). Seven instances across two scan windows before naming, including a fully-drained research queue whose only difference from a dropped one was that it had a durable home. (DataWizard, 2026-08 to 2026-09)
...
**Check whether the rule already exists before minting it.** An enforcement gap or a knowledge gap presents as a policy gap, and the reflex is to write new policy. Twice in one week the proposed rule already existed: a feature request nearly minted cap-and-rotate policy for shell edit_logs that were already exempt under the YAML Schema - the oversized logs were an enforcement failure, not a policy hole (DataWizard, 2026-09) - and a collaborator's fix proposal for a flag-sweep delivery hole asked for a raw-read fallback the sweep's Query Spec already mandated, so the real gap was one new failure signature to document, not a rule to adopt (DataWizard, 2026-09). Before writing a rule, grep for its home: the right fix may be enforcement, an augmentation, or nothing. Kin to "grep the Seed before harvesting into it" and to "verify the drift before scheduling its fix" below. The same reflex generalizes past rules: inspect the ground before building on it - grep the canon before scoping a section, check what the platform natively does before building a classifier, read the live roster instead of trusting a registry file (the tool-inventory and tool-landscape principles above are the tool-shaped members of this family).
