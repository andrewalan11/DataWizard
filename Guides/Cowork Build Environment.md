...
edit_log:
  ...
  - "DW-S331 2026-09-06 - new section: SQLite on the Vault Mount (no-delete
    ...
    positive)"
  - 'DW-S336 2026-09-07 - Device Bridge: commit_files same-path staleness;
    Verification: stub osascript in Mac harness runs'
  - 'DW-S341 2026-09-07 - SQLite/shell: pkill -f self-match on the device shell;
    Device Bridge: background-process survival correction; Verification:
    headless-Chromium recipe for mount-built GUIs'
  - 'DW-S348 2026-09-08 - Device Bridge: desktop-app artifacts have no URL and
...
- **Even inside a quoted heredoc, long single-quoted python string literals can arrive mangled.** A device_bash heredoc python script whose replacement payloads were long parenthesized single-quoted literals failed with a mid-tuple SyntaxError (a quote was eaten in transit); the identical logic with triple-quoted (`"""..."""`) payload strings ran clean. For read-modify-write edit scripts, put every long payload in triple-quoted strings and count-verify each replacement before writing. (Source: VibeCut, 2026-09)
...
The same delete-refusal applies to OTHER FUSE mounts reached through the device bridge (a Dropbox folder connected via folder access): the sqlite3 backup API fails with `disk I/O error` when its DESTINATION is on such a mount (journal lifecycle again). Back up to a scratch path in the VM home first, then `cp` the finished file onto the mount, and verify by round-trip copy + hash. A failed attempt leaves a stub `-journal` on the mount that cannot be deleted - truncate it to zero bytes (`: > file`), which SQLite treats as no journal. (DataWizard, 2026-09)
...
- **`device_commit_files` re-sends the FIRST version of a reused `stagedPath`.** Committing an edited file under an outputs path that was already committed earlier in the session delivered the original bytes again (the call still reported "written"); a test run then exercised the stale script. Write each iteration to a NEW name (`script.v2.sh`, `.v3.sh` ...), commit that, and compare `md5sum` on both sides before running anything. The same-path staleness exists in the staging direction too (MCP Reliability guide, Known Issues). (Source: DataWizard, 2026-09; confirmed Rootland, 2026-09 - a re-commit of edited JSON under a reused outputs path reported "written" and even updated the device mtime, but delivered the original bytes; a fresh staging path fixed it, and cross-side `md5sum` comparison is the only trustworthy check.)
