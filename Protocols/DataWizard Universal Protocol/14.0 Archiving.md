...
Each project has its own archive folder using the `~Archive` prefix, including the project name:

```
_ProjectFolder/~Archive - ProjectName/
```

The `~` prefix sorts the folder to the bottom of the file explorer, keeping it out of the way. The project name makes archives identifiable when searching across the vault. For vault-level files not tied to a specific project, use a root-level archive:
