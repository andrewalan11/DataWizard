---
title: Orientation and Working Sessions
type: project-doc
parent: '[[DataWizard Universal Protocol]]'
section: 2
status: active
---

*Part of the [[DataWizard Universal Protocol]]*

## 3. Orientation vs. Working Sessions

### Lean Orientation (once per thread)

> **Share your plan, get approval, then write to the vault. See Section 4 for the full workflow.**

Each new thread requires a quick orientation. This is designed to minimize token usage while ensuring you're working with current information.

**Step 1: Version check (~10 tokens)**
Fetch the version file from GitHub:
`https://raw.githubusercontent.com/andrewalan11/DataWizard/main/VERSION.md`

This file contains version numbers for `protocol` and `project_instructions`.

**Step 2: Read project guidelines and compare versions**
Read `0.0 Project Guidelines.md` in full. This is the project brief — it tells you what the project is, how it works, its core concepts, key architecture decisions, folder structure, and key pointers. Check:

- `datawizard_protocol_version` in frontmatter vs VERSION.md `protocol` — if match, fetch the **Protocol Summary** (lean refresh). If mismatch, fetch the **full Universal Protocol** and offer to help update the guidelines.
- `# DW Project Instructions v[X]` (or `# DW Cowork Instructions v[X]`) in your system prompt vs VERSION.md `project_instructions` — if mismatch, tell the user their instructions need updating.

Protocol Summary URL: `https://raw.githubusercontent.com/andrewalan11/DataWizard/main/Protocols/Protocol%20Summary.md`

Full Protocol URL: `https://raw.githubusercontent.com/andrewalan11/DataWizard/main/Protocols/DataWizard%20Universal%20Protocol.md`

**Step 3: Read the MOC for file inventory and status**
Read `0.1 MOC.md` for the file inventory and status overview. This gives you a map of the whole project before diving into recent activity. Focus on the file list and status fields — you don't need to deep-read every description.

**Step 4: Read the session log (~100-200 tokens)**
Read `0.2 Session Log.md`. You only need the last 2-3 entries — not the full history. These entries tell you:
- What changed recently (`**Files:**` line)
- Whether there's unfinished work to pick up (`**Status:**` line)
- If Status says "in progress" — read the files listed to pick up where left off
- If Status says "complete" — ready for new work, no extra reads needed

**Step 5: Read action items if they exist**
Check for an action items file (typically `! Action Items - [Project Name].md`). If present, scan it for pending items. This tells you what's queued up beyond what the session log covers. If no action items file exists, skip this step.

**Step 6: Ready to work**
Read additional files only as needed — based on what the session log says changed, or what the human asks you to do. Do not preemptively read the harvest log or other files unless the task requires them.

### Full Orientation (first time in a project)

If this is the first thread for a project — no `0.0 Project Guidelines.md` exists, or the human says this is a new project — do a full orientation:

1. Fetch the full Universal Protocol from GitHub
2. Read it fully
3. Follow Section 18 (Vault Bootstrap) to set up the project
4. Create `0.0 Project Guidelines.md` with `datawizard_protocol_version` set to current
5. Create `0.1 MOC.md`, `0.2 Session Log.md` (shell + section folder), and `0.3 Decision Log.md`

After bootstrap, future threads use the lean orientation flow.

### Each Working Session

1. Share your plan before doing anything. For new content, get approval then write directly to vault. For edits to existing docs, show changes in chat first.
2. Before reading any source file: check its frontmatter with `obsidian:get_frontmatter`. If `harvest_status: harvested` or `harvest_status: reviewed` is present, skip the full content read.
3. At the end of every session, update the session log and MOC. Do this **once per session** — at the end, or at a natural break point if the session is long. Don't update after every individual step. One entry covering the whole session is better than ten micro-entries.
