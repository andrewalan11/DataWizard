...
- **Chunk long session logs.** When a session log grows beyond ~10 entries or ~5000 words, convert it to a shell + section folder. The shell (`0.2 Session Log - Project.md`) contains `![[embed]]` references; session entries live as individual files in `0.2 Session Log - Project/` with filenames like `2026-03-24 - Brief Title.md`. This keeps orientation fast (instances read only the last 2-3 section files) and avoids loading a massive file every thread.
